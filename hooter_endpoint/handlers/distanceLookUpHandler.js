...
const distanceLookUpHandler = {
    'DistanceLookUpIntent': async function () {
        ...
        if (this.event.request.intent.slots.originbuildingname.value){ //If user specifies origin building
            ...
            var userDestAdd = '';
            var destDBApiUrl = 'https://xrmw4eh1gl.execute-api.us-east-1.amazonaws.com/active/building?TableName=building&BuildingName=' + userDest;
            axios.get(destDBApiUrl)
                .then(res => {
                    userDestAdd += res.data.Items[0].address;

                    //Retrieve address for user origin
                    var originDBApiUrl = 'https://xrmw4eh1gl.execute-api.us-east-1.amazonaws.com/active/building?TableName=building&BuildingName=' + userOrigin;
                    var userOriginAdd = ''; 
                    axios.get(originDBApiUrl)
                        .then(res => {
                            ...
                            axios.get(apiurl)
                                .then(res => {
                                    ...
                                    speechOutput += ' to get from ' + userOrigin + ' to ' + userDest;
        ...
        } else { //If user gives only destination
            ...
            if(this.event.context.System.device.supportedInterfaces.Geolocation){
                ...
                if (!geoObject || !geoObject.coordinate) {
                    ...
                    if (geoObject && geoObject.coordinate && geoObject.coordinate.accuracyInMeters < ACCURACY_THRESHOLD && freshness < 60 ) { 
                        ...
                        var geocodingapiurl = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=';
            ...
            } else { // User's device cannot share location
